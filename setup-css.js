...
chrome.storage.sync.get(
  ["classList", "listWidth", "listHeight"],
  function (result) {
    if (result.classList) {
    ...
    if (result.listWidth) {
      document
        ...
        .forEach((el) => (el.style.width = result.listWidth + "px"));
      document
    ...
    if (result.listHeight) {
      document
        .querySelectorAll("#board div.list.js-list-content")
        .forEach((el) => (el.style.maxHeight = result.listHeight + "vh"));
    }
  }
);
